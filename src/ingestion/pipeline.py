...
class IngestionPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            on_progress: Progress callback function(stage, details)
        """
    ...
    def _report_progress(self, stage: str, details: dict):
    ...
    def ingest_file(
        ...
        try:
            ...
            self._report_progress("integrity_check", {"file": str(file_path)})
            ...
            if not config.force_reprocess and self.integrity_checker.should_skip(file_hash):
                result["skipped"] = True
                self._report_progress("skipped", {"reason": "already_processed", "hash": file_hash})
                return result
            ...
            if self.bm25_indexer and config.enable_sparse_encoding:
                ...
                if sparse_records:
                    self._report_progress("store_bm25", {"count": len(sparse_records)})
            ...
            if self.image_storage and "images" in document.metadata:
                ...
                if images:
                    self._report_progress("store_images", {"count": len(images)})
            ...
            self._report_progress("completed", result)
            return result
